...
def dataPreprocessing():
    ...
    # Get the answer and question list from dialogue 
    for conversation in conversations_ids:
        for i in range(len(conversation)-1):
            question.append(id2Line[conversation[i]])
            answer.append(id2Line[conversation[i+1]])

    #re_text the element the the question list and answer list
    for i in range(len(question)-1):
        question[i] = re_text(question[i])
        answer[i] = re_text(answer[i])
...
def sortByLen(questions,answers):
	...
	for length in range(1,25+1):
		...
		for i in enumerate(questions):
			...
			if len(i[1]) == length:
				sortedQuestions.append(questions[0])
				sortedAnswers.append(answers[0])

	return sortedAnswers, sortedQuestions



questions, answers = dataPreprocessing()

questionsWord2Id ,answersWord2Id, Id2Word =  filterTheNonFrequentWord(questions, answers)

questions = addToken(questions)

questions, answers = sentenceEncoding(questions, answers, questionsWord2Id, answersWord2Id)

questions, answers = sortByLen(questions,answers)
